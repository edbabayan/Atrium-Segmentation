...
class CardiacDataset(torch.utils.data.Dataset):
    ...
    def augment(self, slice, mask):
        random_seed = torch.randint(0, 1000, (1,)).item()
        imgaug.seed(random_seed)

        mask = SegmentationMapsOnImage(mask, shape=slice.shape)
    ...
    def __getitem__(self, idx):
        ...
        return np.expand_dims(slice, axis=0), np.expand_dims(mask, axis=0)
